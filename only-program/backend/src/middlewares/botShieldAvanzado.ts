...
export const botShieldAvanzado = async (
  ...
  if (
    ...
    req.path.startsWith("/api/") || // Rutas API del backend — autenticadas con JWT
  ...
  if (VERIFIED_BOTS.some((bot) => ua.includes(bot))) {
    try {
      ...
      const isValid = hostnames.some(
        (h) =>
          h.endsWith(".googlebot.com") ||
          h.endsWith(".google.com") ||
          h.endsWith(".search.msn.com"),
      );

      if (!isValid) {
        // MENTIROSO: Dice ser Googlebot pero no tiene DNS inversa de Google. BLOQUEAR.
        console.log(
          `[BotShield] Fake Bot Detected: ${ip} claiming to be ${ua}`,
        );
  ...
  if (score >= 3) {
    console.log(
      `[BotShield] Challenging Suspicious Client (${score}): ${ip} - ${ua}`,
    );
