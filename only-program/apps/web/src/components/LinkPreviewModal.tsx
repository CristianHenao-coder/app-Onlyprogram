...
const LinkPreviewModal = ({ config, onClose }: LinkPreviewModalProps) => {
    ...
    // Helper to render the preview content based on config
    // We are largely duplicating the logic from MobilePreview to ensure accurate representation
    // Ideally, MobilePreview should be a reusable component receiving props, but if it relies on context, we mock it here.
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="relative w-full max-w-[375px] h-[80vh] bg-black rounded-[3rem] border-8 border-[#333] overflow-hidden shadow-2xl flex flex-col">
                ...
                <div 
                    ...
                    style={{
                        backgroundColor: config.theme === 'custom' ? config.profile.title ? 'transparent' : '#000' : 'var(--theme-bg)', // Fallback
                        // We would apply theme styles here if we had the ThemeContext logic
                    }}
                >   
                     {/* 
                        Since MobilePreview heavily relies on internal state/context in the current codebase, 
                        we are creating a simplified read-only view here for admin moderation.
                     */}
                     ...
                     <div className="flex flex-col items-center pt-16 pb-12 px-6 text-center space-y-6 min-h-full">
                        ...
                        <div className="relative">
                            <div className="h-24 w-24 rounded-full overflow-hidden border-4 border-white/10 shadow-xl">
                                {config.profile.image ? (
                                    ...
                                    <div className="h-full w-full bg-surface/50 flex items-center justify-center">
                                        <span className="material-symbols-outlined text-4xl text-silver/40">person</span>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-sm text-silver/80">{config.profile.bio}</p>
                        </div>
                        ...
                        <div className="w-full space-y-4">
                            {config.buttons.map((btn: any) => (
                                <a 
                                    ...
                                    className="block w-full bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/5 p-4 rounded-xl text-white font-medium transition-all text-center"
