...
export default function Links() {
    ...
    const fetchLinks = async () => {
      try {
        ...
        const dbPages: LinkPage[] = data && data.length > 0 ? data.map(link => ({
          ...
          status: (link.is_active === true ? 'active' : 'draft') as PageStatus,
          name: link.config?.name || link.config?.profile?.name || link.slug,
          profileName: link.config?.profile?.title || link.title || '',
          profileImage: link.config?.profile?.image || link.photo || DEFAULT_PROFILE_IMAGE,
  ...
  useEffect(() => {
    ...
    const syncToDb = async () => {
      ...
      try {
        ...
        const isDraft = currentPageToSave.status === 'draft' && !currentPageToSave.dbStatus;
        ...
        if (isDraft) {
          ...
          const updates = {
            ...
            ...(currentPageToSave.status === 'active' ? { is_active: true } : {}),
  ...
  const handleCreateNew = () => {
    const newId = `page${Date.now()}`;
    setPages(prev => [...prev, { ...DEFAULT_PAGE, id: newId, status: 'draft', name: `Link ${prev.filter(p => !p.dbStatus).length + 1}` }]);
    setSelectedPageId(newId);
    setView('editor');
    toast.success('Empieza a diseñar tu nuevo link');
  };
        ...
        <div className="flex items-center gap-3">
          {view === 'editor' && (
            ...
            >
              ...
              <span className="hidden sm:inline">Mis Links</span>
            </button>
          )}
          <div className="h-8 w-8 rounded-lg bg-primary/20 flex items-center justify-center text-primary">
            <span className="material-symbols-outlined text-lg">{view === 'editor' ? 'edit' : 'layers'}</span>
          </div>
          <h1 className="text-sm font-bold uppercase tracking-wider">
            {view === 'editor' ? (currentPage?.name || 'Editor de Link') : t('dashboard.links.managerTitle')}
          </h1>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-8">
              <div>
                <h2 className="text-2xl font-black text-white">Mis Links</h2>
                <p className="text-sm text-silver/40 mt-1">{pages.filter(p => p.dbStatus).length} link(s) creados · {pages.filter(p => !p.dbStatus && p.status === 'draft').length} borrador(es)</p>
              </div>
              <button
            ...
            {pages.filter(p => p.dbStatus).length > 0 && (
              <div className="mb-8">
                <h3 className="text-xs font-bold uppercase tracking-wider text-silver/40 mb-4">Links Enviados</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {pages.filter(p => p.dbStatus).map(page => (
                    <div
                      key={page.id}
                    ...
                    >
                      ...
                      <div className="pt-9 pb-4 px-4 text-center">
                        <p className="font-bold text-sm text-white truncate">{page.profileName || page.name}</p>
                        <p className="text-[10px] text-silver/40 mt-0.5">/{page.slug || page.id.slice(0, 8)}</p>
                        ...
                        <div className="mt-2 flex items-center justify-center gap-1.5">
                          {page.status === 'active' ? (
                            <>
                              ...
                              <span className="text-[9px] font-black uppercase tracking-wider text-green-500">Activo</span>
                            </>
                          ) : (
                            <>
                              <span className="w-1.5 h-1.5 rounded-full bg-orange-400 animate-pulse" />
                              <span className="text-[9px] font-black uppercase tracking-wider text-orange-400">En revisión</span>
                            </>
                          )}
                        </div>
            ...
            {pages.filter(p => !p.dbStatus && p.status === 'draft').length > 0 && (
              <div>
                <h3 className="text-xs font-bold uppercase tracking-wider text-silver/40 mb-4">Borradores (no enviados)</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {pages.filter(p => !p.dbStatus && p.status === 'draft').map(page => (
                    <div
                      key={page.id}
                    ...
                    >
                        ...
                        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2">
                          <div className="w-14 h-14 rounded-full border-2 border-white/20 overflow-hidden bg-gray-800">
                            {page.profileImage && page.profileImage !== DEFAULT_PROFILE_IMAGE ? (
                              ...
                              <div className="w-full h-full flex items-center justify-center bg-white/5">
                                <span className="material-symbols-outlined text-2xl text-white/20">edit</span>
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="pt-9 pb-4 px-4 text-center">
                        <p className="font-bold text-sm text-white/70 truncate">{page.name || 'Sin nombre'}</p>
                        <div className="mt-2 flex items-center justify-center gap-1.5">
                          <span className="text-[9px] font-black uppercase tracking-wider text-yellow-500">Borrador</span>
                        </div>
                        <div className="mt-3">
                          <button
                            onClick={() => openEditor(page.id)}
            ...
            {pages.length === 0 && (
              <div className="text-center py-20">
                <span className="material-symbols-outlined text-6xl text-white/10">link</span>
                <p className="text-silver/40 mt-4">No tienes links aún</p>
                <button onClick={handleCreateNew} className="mt-4 px-6 py-3 bg-primary rounded-xl font-bold text-sm hover:scale-105 transition-all">
                  ...
                  >
                    ...
                    <div className="text-left min-w-[60px]">
                      ...
                      {page.dbStatus ? (
                        <span className="inline-flex items-center gap-1 mt-0.5">
                          <span className="w-1.5 h-1.5 rounded-full bg-orange-400 animate-pulse" />
                          <span className="text-[9px] text-orange-400 font-black uppercase tracking-wider">En revisión</span>
                        </span>
                      ) : (
                          ...
                          <div className="space-y-8 animate-fade-in">
                            ...
                            <section className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
                              <div className="p-4 border-b border-white/5 bg-white/[0.02] flex justify-between items-center">
                                ...
                                <div className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${currentPage.status === 'active' ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-yellow-500/10 text-yellow-500 border border-yellow-500/20'
                                  }`}>
