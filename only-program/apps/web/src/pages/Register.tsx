...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    const { data, error } = await signUpWithEmail(email, password, undefined, captchaToken || undefined);
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-background-dark p-4">
        <div className="bg-surface border border-border rounded-2xl p-8 max-w-md w-full text-center shadow-2xl">
          ...
          <p className="text-silver mb-8">
            ...
            Si no recibes el correo, ve a tu panel de Supabase &gt; Auth &gt; Providers &gt; Email y desactiva "Confirm email". Luego intenta registrarte de nuevo.
          </p>
          <Link
            to="/login"
            className="inline-block w-full bg-primary hover:bg-primary-dark text-white font-bold py-3 rounded-xl transition-colors"
          >
            Ir al Login
          </Link>
        </div>
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden bg-background-dark">
      {/* Background Glows */}
      <div className="absolute top-[-10%] right-[-10%] w-[40%] h-[40%] bg-glow pointer-events-none"></div>
      <div className="absolute bottom-[-10%] left-[-10%] w-[40%] h-[40%] bg-glow pointer-events-none"></div>
      ...
      <main className="w-full max-w-md px-4 z-10">
        ...
        <div className="bg-surface border border-border rounded-2xl p-8 md:p-10 shadow-2xl backdrop-blur-sm">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-silver text-sm">Únete a la plataforma más segura para tus enlaces</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-5">
            ...
            <div className="relative py-2">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-surface px-4 text-silver/40">O registrarse con</span>
              </div>
            </div>
