...
export default function Login() {
   ...
   */
  const checkDeviceTrusted = async (userId: string): Promise<boolean> => {
  ...
  const handlePasswordLogin = async (e: React.FormEvent) => {
    ...
    if (data.user) {
      ...
      const trusted = await checkDeviceTrusted(data.user.id);

      if (trusted) {
        // Skip verification for known device
        await handleProfileRedirect(data.user.id);
      } else {
        // Need to verify via OTP first
        // Send OTP
        try {
          const otpResponse = await fetch(`${API_URL}/auth/request-otp`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ email, usage: 'login', lang }),
          });
          const otpData = await otpResponse.json();

          if (!otpResponse.ok) {
            setError(otpData.error || 'No se pudo enviar el código de verificación');
            await supabase.auth.signOut();
            setLoading(false);
            return;
          }

          // Redirect to verification with user context
          navigate('/auth/verify', {
            state: { email, userId: data.user.id, usage: 'login' },
          });
        } catch {
      ...
      >
        ...
        <div className="flex bg-white/5 p-1 rounded-2xl mb-8">
          <button
            ...
            className={`flex-1 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${loginMode === 'password' ? 'bg-primary text-white' : 'text-silver/40 hover:text-white'}`}
          >
            {t('auth.password')}
          </button>
          <button
            onClick={() => setLoginMode('otp')}
            className={`flex-1 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${loginMode === 'otp' ? 'bg-primary text-white' : 'text-silver/40 hover:text-white'}`}
          >
            Código OTP
          </button>
        </div>
          ...
          {loginMode === 'password' ? (
            ...
            >
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-primary text-white font-black py-5 rounded-2xl transition-all shadow-lg shadow-primary/20 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed mt-6 uppercase tracking-widest text-sm hover:scale-[1.02] active:scale-95"
              >
                {loading ? (
                  <span className="flex items-center gap-2">
                    <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
