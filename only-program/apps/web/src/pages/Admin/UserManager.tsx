...
const UserManager = () => {
  ...
  // Función helper: fecha más reciente de los links del usuario
  const getLastLinkDate = (u: any): number => {
    if (!u.smart_links || u.smart_links.length === 0) return new Date(u.created_at).getTime();
    const latest = Math.max(...u.smart_links.map((l: any) => new Date(l.created_at).getTime()));
    return latest;
  };
  ...
  const filteredUsers = users
    ...
    // Ordenar: el que hizo la compra más reciente primero
    .sort((a, b) => getLastLinkDate(b) - getLastLinkDate(a));
      ...
      <div className="bg-surface/30 border border-border/50 rounded-3xl overflow-hidden shadow-2xl">
        <table className="w-full text-left border-collapse">
          <thead className="bg-white/5 border-b border-border/50">
            <tr>
              <th className="px-6 py-4 text-[10px] font-black text-white uppercase tracking-widest text-center">ID / Fecha</th>
              <th className="px-6 py-4 text-[10px] font-black text-white uppercase tracking-widest">Cliente</th>
              <th className="px-6 py-4 text-[10px] font-black text-white uppercase tracking-widest text-center">Total Links</th>
              <th className="px-6 py-4 text-[10px] font-black text-white uppercase tracking-widest text-center">Activos</th>
              <th className="px-6 py-4 text-[10px] font-black text-white uppercase tracking-widest text-center">Inversión</th>
              <th className="px-6 py-4 text-[10px] font-black text-white uppercase tracking-widest text-center">Ver Links</th>
              <th className="px-6 py-4 text-[10px] font-black text-white uppercase tracking-widest text-right">Acciones</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border/20">
            {filteredUsers.map((user) => {
              const displayName = user.full_name || user.email?.split('@')[0] || user.id.slice(0, 8);
              const totalLinks = user.smart_links?.length || 0;
              const activeLinks = user.smart_links?.filter((l: any) => l.is_active).length || 0;
              const isExpanded = expandedUsers.has(user.id);
              // Fecha más reciente de compra (último link creado)
              const lastLinkDate = getLastLinkDate(user);
              const lastLinkDateStr = new Date(lastLinkDate).toLocaleDateString('es-CO', { day: '2-digit', month: 'short', year: 'numeric' });
              // Links ordenados del más reciente al más antiguo
              const sortedLinks = [...(user.smart_links || [])].sort((a: any, b: any) =>
                new Date(b.created_at).getTime() - new Date(a.created_at).getTime()
              );
              return (
                <Fragment key={user.id}>
                  <tr className={`group transition-colors relative z-10 ${isExpanded ? 'bg-primary/5 border-b border-primary/10' : 'hover:bg-white/[0.02] bg-surface/30'}`}>
                    {/* ID + Fecha última compra */}
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1">
                        <p className="text-[10px] text-silver font-mono bg-white/5 px-2 py-1 rounded-md max-w-[110px] truncate" title={user.id}>{user.id}</p>
                        <div className="flex items-center gap-1 pl-1">
                          <span className="material-symbols-outlined text-[10px] text-primary/60">shopping_bag</span>
                          <p className="text-[9px] text-primary/70 font-mono font-bold">
                            {lastLinkDateStr}
                          </p>
                        </div>
                      </div>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-bold text-white max-w-[160px] truncate" title={user.full_name || user.email}>
                            {displayName}
                          </p>
                          {user.email && (
                            <p className="text-[10px] text-silver/50 font-mono max-w-[160px] truncate" title={user.email}>
                              {user.email}
                            </p>
                          )}
                          {user.is_suspended && (
                            <span className="text-[9px] font-black text-red-400 uppercase tracking-wider">Suspendido</span>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    {/* Activos */}
                    <td className="px-6 py-4 text-center">
                      <span className={`px-3 py-1 rounded-full text-xs font-black border ${activeLinks > 0
                        ? 'bg-primary/20 text-primary border-primary/30'
                        : 'bg-white/5 text-silver/30 border-white/10'
                        }`}>
                        {activeLinks}
                      </span>
                    </td>
                    {/* Inversión */}
                    <td className="px-6 py-4 text-center">
                      <p className={`text-sm font-black ${activeLinks > 0 ? 'text-green-400' : 'text-silver/30'}`}>
                        ${activeLinks * 15} USD
                      </p>
                    </td>
                    {/* Ver Links Button */}
                    <td className="px-6 py-4 text-center">
                      <button
                        ...
                        className={`flex items-center justify-center gap-1.5 mx-auto px-4 py-2 rounded-xl font-bold text-xs transition-all border ${isExpanded
                          ? 'bg-primary text-black border-primary shadow-[0_0_15px_rgba(var(--primary-rgb),0.4)]'
                          : 'bg-white/5 border-white/10 text-silver hover:bg-primary/10 hover:border-primary/30 hover:text-primary'
                          }`}
                      >
                        <span className="material-symbols-outlined text-sm">{isExpanded ? 'expand_less' : 'account_tree'}</span>
                        {isExpanded ? 'Ocultar' : 'Ver Links'}
                      </button>
                    </td>
                    {/* Acciones */}
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className={`h-8 w-8 rounded-xl flex items-center justify-center flex-shrink-0 transition-all border ${user.is_suspended
                        ...
                        <button
                          ...
                          className="h-8 w-8 rounded-xl flex items-center justify-center flex-shrink-0 bg-red-500/10 border border-red-500/20 text-red-500 hover:bg-red-500 hover:text-white transition-all"
                        >
                                ...
                                <div key={link.id} className="relative flex flex-col md:flex-row md:items-center gap-4 group">
                                  ...
                                  <div className="w-full bg-white/[0.03] hover:bg-white/[0.05] border border-white/5 hover:border-primary/20 rounded-xl p-3 flex flex-wrap items-center justify-between gap-4 transition-all group-hover:translate-x-1">
                                    ...
                                    <div className="flex-1 flex flex-wrap items-center gap-6 justify-between md:justify-end">
                                      ...
                                      <div className="text-center min-w-[80px]">
                                        <span className="text-[10px] text-silver/40 font-bold block">Fecha Creación</span>
                                        <span className="text-[11px] font-mono text-silver">{new Date(link.created_at).toLocaleDateString('es-CO', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                                      </div>
